...
(function ($) {
            ...
            if (!$.string.isNullOrWhiteSpace(str)) {
                $.ajax({
                    ...
                    success: function (data) {
                        if ($.isFunction(selectedFilter)) { data = selectedFilter.call(datagrid2[0], data, selectedArr); }
    ...
    function initialize(options) {
        ...
        var container = dia.find(".grid-selector-container"),
            ...
            btn1 = datagridOptions.singleSelect ? null : $("<a />").linkbutton({ plain: true, iconCls: "pagination-last" }).tooltip({ content: "选择全部" }).appendTo(centerPanel),
            btn2 = datagridOptions.singleSelect ? null : $("<a />").linkbutton({ plain: true, iconCls: "pagination-first" }).tooltip({ content: "取消全部" }).appendTo(centerPanel);
